...
metadata:
  labels:
    jenkins/label: sgs-react-tools-build
spec:
  containers:
    - name: node
      image: node:24-slim
...
'''
    ...
    parameters {
        ...
        string(name: 'CONTAINER_ENV_FILE', defaultValue: '/opt/sgs-react-tools/.env', description: 'Env file path on OVH server')
        ...
        string(name: 'OVH_SSH_KEY_CREDENTIALS_ID', defaultValue: 'server-ssh-key', description: 'Jenkins SSH key credential ID for OVH server')
    }
            ...
            steps {
                container('docker') {
                    withCredentials([
                        ...
                        sshUserPrivateKey(credentialsId: params.OVH_SSH_KEY_CREDENTIALS_ID, keyFileVariable: 'SSH_KEY', usernameVariable: 'SSH_USER'),
